...
enum CLASSES{STOP=0, YIELD=1, SIGNALAHEAD=2};
...
void initGlobals () {
  CLASS_NAMES.push_back("stop");
  CLASS_NAMES.push_back("yield");
  CLASS_NAMES.push_back("signalahead");
}
...
vector<tDetection> loadDetections(string file_name, bool &compute_aos, bool &eval_stop, bool &eval_yield, bool &eval_signalahead, bool &success) {

  // holds all detections (ignored detections are indicated by an index vector
  vector<tDetection> detections;
  printf("file name3: %s", file_name.c_str());
  FILE *fp = fopen(file_name.c_str(),"r");
  if (!fp) {
    success = false;
    printf("Detection file open fail\n");
    return detections;
  }
  while (!feof(fp)) {
    tDetection d;
    ...
    if (fscanf(fp, "%s %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf",
      ...
      if(!eval_stop && !strcasecmp(d.box.type.c_str(), "stop"))
        eval_stop = true;
      if(!eval_yield && !strcasecmp(d.box.type.c_str(), "yield"))
        eval_yield = true;
      if(!eval_signalahead && !strcasecmp(d.box.type.c_str(), "signalahead"))
        eval_signalahead = true;
    }
  }
  fclose(fp);
  success = true;
...
vector<tGroundtruth> loadGroundtruth(string file_name,bool &success) {
  ...
  printf("file name2: %s", file_name.c_str());
  FILE *fp = fopen(file_name.c_str(),"r");
  if (!fp) {
    success = false;
    printf("fopen fails\n");
...
//  string result_dir     = "results/" + result_sha;
  ...
  bool compute_aos=true, eval_stop=false, eval_yield=false, eval_signalahead=false;
  ...
  for (int32_t i=0; i<N_TESTIMAGES; i++) {
    ...
    printf("gt_dir=%s\n", gt_dir.c_str());
    printf("file_name=%s\n", file_name);
    printf("ospj=%s\n", ospj(gt_dir, file_name).c_str());
    vector<tGroundtruth> gt   = loadGroundtruth(ospj(gt_dir,file_name),gt_success);

    //vector<tDetection>   det  = loadDetections(ospj(result_dir,"data",file_name), compute_aos, eval_stop, eval_yield, eval_signalahead,det_success);
    vector<tDetection>   det  = loadDetections(ospj(result_dir,file_name), compute_aos, eval_stop, eval_yield, eval_signalahead,det_success);
  ...
  // eval cars
  if(eval_stop){
    fp_det = fopen((result_dir + "/stats_" + CLASS_NAMES[STOP] + "_detection.txt").c_str(),"w");
    fp_ap = fopen((result_dir + "/stats_" + CLASS_NAMES[STOP] + "_ap.txt").c_str(),"w");
    if(compute_aos)
      fp_ori = fopen((result_dir + "/stats_" + CLASS_NAMES[STOP] + "_orientation.txt").c_str(),"w");
    vector<double> precision[3], aos[3];
    if( !eval_class(fp_det,fp_ap,fp_ori,STOP,groundtruth,detections,compute_aos,precision[0],aos[0],EASY,N_TESTIMAGES)
       || !eval_class(fp_det,fp_ap,fp_ori,STOP,groundtruth,detections,compute_aos,precision[1],aos[1],MODERATE, N_TESTIMAGES)
       || !eval_class(fp_det,fp_ap,fp_ori,STOP,groundtruth,detections,compute_aos,precision[2],aos[2],HARD, N_TESTIMAGES)){
      mail->msg("Car evaluation failed.");
      return false;
    }
    fclose(fp_det);
    fclose(fp_ap);
    saveAndPlotPlots(plot_dir,CLASS_NAMES[STOP] + "_detection",CLASS_NAMES[STOP],precision,0);
    if(compute_aos){
      saveAndPlotPlots(plot_dir,CLASS_NAMES[STOP] + "_orientation",CLASS_NAMES[STOP],aos,1);
      fclose(fp_ori);
    }
  }
  ...
  // eval pedestrians
  if(eval_yield){
    fp_det = fopen((result_dir + "/stats_" + CLASS_NAMES[YIELD] + "_detection.txt").c_str(),"w");
    fp_ap = fopen((result_dir + "/stats_" + CLASS_NAMES[YIELD] + "_ap.txt").c_str(),"w");
    if(compute_aos)
      fp_ori = fopen((result_dir + "/stats_" + CLASS_NAMES[YIELD] + "_orientation.txt").c_str(),"w");
    vector<double> precision[3], aos[3];
    if( !eval_class(fp_det,fp_ap,fp_ori,YIELD,groundtruth,detections,compute_aos,precision[0],aos[0],EASY, N_TESTIMAGES)
       || !eval_class(fp_det,fp_ap,fp_ori,YIELD,groundtruth,detections,compute_aos,precision[1],aos[1],MODERATE,N_TESTIMAGES)
       || !eval_class(fp_det,fp_ap,fp_ori,YIELD,groundtruth,detections,compute_aos,precision[2],aos[2],HARD,N_TESTIMAGES)){
      mail->msg("Pedestrian evaluation failed.");
      return false;
    }
    fclose(fp_det);
    fclose(fp_ap);
    saveAndPlotPlots(plot_dir,CLASS_NAMES[YIELD] + "_detection",CLASS_NAMES[YIELD],precision,0);
    if(compute_aos){
      fclose(fp_ori);
      saveAndPlotPlots(plot_dir,CLASS_NAMES[YIELD] + "_orientation",CLASS_NAMES[YIELD],aos,1);
    }
  }

  // eval cyclists
  if(eval_signalahead){
    fp_det = fopen((result_dir + "/stats_" + CLASS_NAMES[SIGNALAHEAD]  + "_detection.txt").c_str(),"w");
    fp_ap = fopen((result_dir + "/stats_" + CLASS_NAMES[SIGNALAHEAD] + "_ap.txt").c_str(),"w");
    if(compute_aos)
      fp_ori = fopen((result_dir + "/stats_" + CLASS_NAMES[SIGNALAHEAD] + "_orientation.txt").c_str(),"w");
    vector<double> precision[3], aos[3];
    if( !eval_class(fp_det,fp_ap,fp_ori,SIGNALAHEAD,groundtruth,detections,compute_aos,precision[0],aos[0],EASY, N_TESTIMAGES)
       || !eval_class(fp_det,fp_ap,fp_ori,SIGNALAHEAD,groundtruth,detections,compute_aos,precision[1],aos[1],MODERATE, N_TESTIMAGES)
       || !eval_class(fp_det,fp_ap,fp_ori,SIGNALAHEAD,groundtruth,detections,compute_aos,precision[2],aos[2],HARD, N_TESTIMAGES)){
      mail->msg("Cyclist evaluation failed.");
      return false;
    }
    fclose(fp_det);
    fclose(fp_ap);
    saveAndPlotPlots(plot_dir,CLASS_NAMES[SIGNALAHEAD] + "_detection",CLASS_NAMES[SIGNALAHEAD],precision,0);
    if(compute_aos){
      fclose(fp_ori);
      saveAndPlotPlots(plot_dir,CLASS_NAMES[SIGNALAHEAD] + "_orientation",CLASS_NAMES[SIGNALAHEAD],aos,1);
    }
  }
